  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchTypes.fulfilled, (state, action) => {
        ...
        state.types = action.payload.filter(t => !['unknown', 'shadow'].includes(t.name));
      })
      ...
      .addCase(fetchPokemon.fulfilled, (state, action) => {
        ...
        if (action.payload.isAllContent) {
          if (action.meta.arg.page === 0) {
            ...
            const newResults = action.payload.results.filter((p: any) => !existingNames.has(p.name));
...
export const { setSelectedType, setViewMode, setSearchTerm, setPage } = pokemonSlice.actions;
