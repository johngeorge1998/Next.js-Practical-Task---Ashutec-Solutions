...
export function PokemonDisplay() {
  ...
  useEffect(() => {
    const promise = dispatch(
      fetchPokemon({ type: selectedType, page, limit: ITEMS_PER_PAGE }),
    );
  ...
  return (
    <div
      ...
      className="flex-1 overflow-y-auto w-full p-4 md:p-8 bg-background relative flex flex-col h-full scroll-smooth"
    >
      <div className="flex items-center justify-between mb-6">
        ...
        {status !== "loading" && (
          <span className="text-sm font-medium text-muted-foreground bg-secondary/50 px-3 py-1 rounded-full">
            ...
            {filteredPokemon.length === 1 ? "Result" : "Results"}
          </span>
        )}
      </div>
